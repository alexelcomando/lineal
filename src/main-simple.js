/**
 * Versión Simplificada - Editor Inteligente de Imágenes
 * Sin dependencias externas, JavaScript vanilla puro
 */

/**
 * Clase simplificada de Matrix para reemplazar ml-matrix
 */
class SimpleMatrix {
    constructor(data) {
...
/**
 * Operaciones matriciales simplificadas
 */
class MatrixOperations {
    ...
    static normalize(matrix) {
        ...
        if (range === 0) {
            return new SimpleMatrix({rows: matrix.rows, columns: matrix.columns}).fill(0);
        }
...
/**
 * Operaciones vectoriales simplificadas
 */
...
class LinearTransformations {
    ...
    static transformImage(imageMatrix, transformMatrix, width, height) {
        const transformedMatrix = new SimpleMatrix({rows: imageMatrix.rows, columns: imageMatrix.columns}).fill(0);
...
/**
 * Cargador de imágenes simplificado
 */
class ImageLoader {
...
/**
 * Filtros de imagen simplificados
 */
class ImageFilters {
    ...
    static applyEdgeDetection(imageMatrix, threshold = 50) {
        ...
        const magnitude = new SimpleMatrix({rows: imageMatrix.rows, columns: imageMatrix.columns});
    ...
    static applyGrayscale(matrices) {
        ...
        return {
            ...
            alpha: matrices.alpha ? matrices.alpha.clone() : new SimpleMatrix({rows: grayscaleMatrix.rows, columns: grayscaleMatrix.columns}).fill(255),
            grayscale: grayscaleMatrix
        };
    }
    ...
    static convolve(imageMatrix, kernel) {
        ...
        const result = new SimpleMatrix({rows: imageMatrix.rows, columns: imageMatrix.columns});
...
/**
 * Clase principal del Editor Simplificado
 */
