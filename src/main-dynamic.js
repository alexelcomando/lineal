/**
 * VERSIÓN DINÁMICA - ADAPTA AL TAMAÑO REAL DE LAS IMÁGENES
 * Editor Inteligente de Imágenes - Álgebra Lineal
 */

console.log('Cargando editor de imágenes dinámico...');
...
let imageWidth = 400;
let imageHeight = 400;
let maxCanvasSize = 800; // Máximo tamaño para mostrar en pantalla

// ==========================================
// FUNCIONES DE MANEJO DE TAMAÑO
// ==========================================
function calculateOptimalCanvasSize(width, height) {
    // Calcular el tamaño óptimo manteniendo la proporción
    const maxDimension = Math.max(width, height);
    const scale = Math.min(maxCanvasSize / maxDimension, 1);
...
function resizeCanvas(canvasId, width, height) {
    ...
    const currentContent = canvas.width > 0 ? canvas.toDataURL() : null;
    ...
    if (currentContent && currentImageData) {
        const img = new Image();
        img.onload = () => {
    ...
    console.log(`Canvas ${canvasId} redimensionado a ${width}x${height}`);
    return true;
}
...
function updateCanvasSize() {
    ...
    const optimalSize = calculateOptimalCanvasSize(originalImageData.width, originalImageData.height);
    
    // Redimensionar ambos canvas
    resizeCanvas('originalCanvas', optimalSize.width, optimalSize.height);
    resizeCanvas('transformedCanvas', optimalSize.width, optimalSize.height);
    
    // Mostrar información del tamaño
    showImageInfo(originalImageData.width, originalImageData.height, optimalSize.scale);
}

function showImageInfo(originalWidth, originalHeight, scale) {
    const infoDiv = document.getElementById('imageInfo') || createImageInfoDiv();
    
    infoDiv.innerHTML = `
        <strong>Info Imagen:</strong><br>
        Original: ${originalWidth}x${originalHeight}px<br>
        Escala: ${(scale * 100).toFixed(1)}%<br>
        Mostrando: ${Math.round(originalWidth * scale)}x${Math.round(originalHeight * scale)}px
    `;
...
function createSampleImage(width, height) {
    ...
    // Crear gradiente más interesante
    ...
    // Agregar algunos círculos para hacerla más interesante
...
// ==========================================
// FUNCIONES DE TRANSFORMACIÓN (AHORA DINÁMICAS)
// ==========================================
...
async function handleFileUpload(event) {
    ...
    try {
        ...
        // Actualizar tamaño de canvas dinámicamente
        updateCanvasSize();
        
        // Dibujar en ambos canvas
        const success1 = drawImageDataToCanvas(imageData, 'originalCanvas');
        const success2 = drawImageDataToCanvas(imageData, 'transformedCanvas');
...
// ==========================================
// FUNCIONES PRINCIPALES (AHORA DINÁMICAS)
// ==========================================
function loadSampleImage() {
    try {
        console.log('Cargando imagen de ejemplo...');
        
        // Crear imagen de ejemplo con tamaño variable
...
// ==========================================
function startApp() {
    console.log('Iniciando aplicación dinámica...');
    ...
    console.log('Aplicación dinámica iniciada correctamente');
}
...
console.log('Script dinámico cargado');
